  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録できない場合' do
      ...
      it 'last_nameが全角（漢字・ひらがな・カタカナ）以外では登録できない' do
        @user.last_name = 'ﾃｽﾄ'
        @user.valid?
        expect(@user.errors.full_messages).to include('お名前(姓)は全角文字を入力してください')
      end
      it 'first_nameが全角（漢字・ひらがな・カタカナ）以外では登録できない' do
        @user.first_name = 'shimasu'
        @user.valid?
        expect(@user.errors.full_messages).to include('お名前(名)は全角文字を入力してください')
      end
      it 'last_name_readingが全角（カタカナ）以外では登録できない' do
        @user.last_name_reading = 'てすと'
        @user.valid?
        expect(@user.errors.full_messages).to include('お名前カナ(姓)は全角カナ文字を入力してください')
      end
      it 'first_name_readingが全角（カタカナ）以外では登録できない' do
        @user.first_name_reading = 'ｼﾏｽ'
        @user.valid?
        expect(@user.errors.full_messages).to include('お名前カナ(名)は全角カナ文字を入力してください')
      end
