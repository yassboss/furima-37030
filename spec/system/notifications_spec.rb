...
RSpec.describe "通知", type: :system do
  before do
    ...
    @notification = FactoryBot.create(:notification, visitor_id: @comment.user_id, visited_id: @item.user_id, item_id: @item.id, comment_id: @comment.id)
  end
  context '通知確認' do
    it '出品者以外のユーザーがコメントすると通知が作成され出品者が確認できる' do
      ...
      find_link("#{@item.item_name}").click
