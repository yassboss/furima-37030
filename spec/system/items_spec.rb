...
RSpec.describe '商品出品', type: :system do
  ...
  context '商品出品ができるとき' do
    it 'ログインしたユーザーは商品出品できる' do
      ...
      visit new_item_path
      # フォームに情報を入力する
      attach_file 'item[images][]', 'public/images/test_image.png', make_visible: true
      fill_in 'item-name', with: 'test_item_name'
      fill_in 'item-info', with: 'test_item_description'
      ...
      expect(page).to have_content('test_item_name')
...
RSpec.describe '商品編集', type: :system do
  before do
    @item_01 = FactoryBot.create(:item, :assoc)
    @item_02 = FactoryBot.create(:item, :assoc)
  end
  context '商品編集ができるとき' do
    it 'ログインしたユーザーは自分が出品した商品の編集ができる' do
      # 商品01を投稿したユーザーでログインする
      visit new_user_session_path
      fill_in 'email', with: @item_01.user.email
      fill_in 'password', with: @item_01.user.password
      find('input[name="commit"]').click
      # 商品01の詳細ページに遷移する
      visit item_path(@item_01)
      # 商品01に「編集」へのリンクがあることを確認する
      expect(page).to have_content('商品の編集')
      # 編集ページへ遷移する
      visit edit_item_path(@item_01)
      # すでに出品済みの内容がフォームに入っていることを確認する
      expect(page).to have_selector('img')
      expect(page).to have_content(@item_01.item_name)
      expect(page).to have_content(@item_01.shipping_charge.name)
      expect(find('#item-price').value).to eq(@item_01.price.to_s)
      expect(page).to have_content(@item_01.description)
      expect(page).to have_content(@item_01.category.name)
      expect(page).to have_content(@item_01.condition.name)
      expect(page).to have_content(@item_01.prefecture.name)
      expect(page).to have_content(@item_01.days_to_ship.name)
      # 商品内容を編集する
      ...
      # 商品詳細ページに遷移することを確認する
      expect(current_path).to eq(item_path(@item_01))
...
RSpec.describe '商品削除', type: :system do
  ...
  context '商品削除ができるとき' do
    it 'ログインしたユーザーは自らが出品した商品の削除ができる' do
      ...
      expect do
        find_link('削除', href: item_path(@item_01)).click
      end.to change { Item.count }.by(-1)
      # トップページに遷移したことを確認する
      expect(current_path).to eq(root_path)
      ...
      expect(page).to have_no_content(@item_01.item_name)
    end
  end
